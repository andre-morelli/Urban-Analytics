...
def calc_zone_accessibility(zones, G, opportunities_column=None,
                             pois=None, pois_column=None,
                             weight='length', func=acc_cumulative_gaussian,
                             func_kws={},k=5, random_seed=None, 
                             iter_cap=1_000, round_trip = False,
                             competition = True,pop_column=None,
                             competition_kws=None,node_subset=None,
                             competition_graph=None):
    """
    Calculate accessibility by census zone using given accessibility function.
    ...
    if competition:
        assert pop_column is not None, 'needs a pop_column for copetition analysis'
    ...
    if pois is not None:
        ...
        if pois_column is None:
            pois_column = 'temp'
            pois = pois.copy()
            pois[pois_column] = 1
        for node, val in zip(nodes,pois[pois_column]):
            attrs[node] += val
        nx.set_node_attributes(G,attrs,pois_column)    
        
        #get nodes to target (for faster shortest paths)
        n_targets = [n for n in G.nodes if G.nodes[n][pois_column]>0]
        nig_targets = [node_dict[n] for n in n_targets]
        vals = [G.nodes[n][pois_column] for n in n_targets]
        
        if competition:
            pop_targs = []
            pops_of_targs = []
            #avoid duplicates
            for n,v in enumerate(zones[pop_column]):
        ...
        if competition:
            ...
            opp_acc = calc_zone_accessibility(zones, competition_graph, opportunities_column=pop_column,
                                              weight=weight, func=func,
                                              func_kws=competition_kws,k=k, random_seed=random_seed, 
                                              iter_cap=iter_cap, round_trip = round_trip,
                                              competition = False)
...
def accessibility_load(tracts, G, pois=None, weight='length',
                            func=acc_cumulative_gaussian, k=5,
                            random_seed=None,func_kws={},
                            zone_pois_column=None,node_subset=None,
                            pois_weight_column=None,
                            tracts_weight_column=None,
                            track_progress=False,norm=False,
                            write_name='load',competition=True,
                            competition_kws=None,
                            competition_graph=None):
    """
        ...
        for n in range(len(tracts)):
            ...
            if len(nns)==0: 
                ...
                for nn in nns:
                    ...
                    if pois is None:
                        v=list(tracts[zone_pois_column])[n]
                        vals[i]+=v/k
            else:
                pop_targs+=nns
